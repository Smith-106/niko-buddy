...
## 4. 人工完成情况（2026-09-12 更新）

### 4.1 视觉核对：**已完成（用户判定通过）**

用户在两个**独立渲染器**上打开单页中文样本目视：**WPS PDF**（系统默认处理器 `KWPS.PDF.9`）与 **Edge**（Chromium 内置 PDF，pdfium）。

| 目视项 | 用户判定 |
|---|---|
| 中文字形（乱码方块 / 缺字 / 叠字 / 字重） | 无问题 |
| 中文标点（`，。、""`）与拉丁数字观感 | 正常 |
| 行距观感 | 不挤不散；交付样本为**单行段落**，故目视可见的是**段间距约两倍行距**（段内 1.5 行距由 §2.1 机械量测覆盖：16.54 / 16.51 / 16.88 pt） |
| 版式（页数 / 多余空白） | 仅 1 页，无多余空白 |
| 跨阅读器一致性（WPS vs Edge） | 一致 |

用户结论原话：**「没问题」**。

**未单独确认**（故不随本次判为通过）：选中复制后的文本是否出现康熙部首映射（`舟→U+2F88`、`门→U+2ED4`、`色→U+2F8A`、`里→U+2F8B`）。该项**只影响复制与检索、不影响视觉**，已在 §3 与本仓 `smoke-report.md` §3 登记；不随本次目视判为通过。

### 4.2 仍未由人完成 / 未覆盖的部分

- **跨阅读器全量核对**：已在 WPS 与 Edge 两个渲染器上完成一致性与观感核对（§4.1）；**未**在 Acrobat / 福昕 / SumatraPDF 等其余阅读器上逐一体检。
- **复制/检索回读路径**：见 §4.1 「未单独确认」；如需用于检索，先做康熙部首归一化（NFKC 或等价映射表）。

### 4.3 自动化视觉通路失败记录（留档，不代表视觉结论）

本节结论**已由用户目视替代**；以下记录仅说明「为何本项必须由人完成」，保留为环境证据。

1. **第一次尝试（三条通路全败）**
   - 本机多模态助手（`describe_image`）：路径解析正常（文件不存在时报 ENOENT），但对存在的 PNG 两次均返回「没有收到图片」——图片附件管道未把像素送达模型。
   - 视觉能力队友（`glm-5.3-flash`，[vision]）：6 次工具调用全部 `Permission relay send-failed: write EPIPE`，连 `ls` 都无法执行；该队友**明确拒绝编造**图像内容。
   - 外部 CLI 委派（`maestro delegate --to gemini`）：实际路由到 codex，上游 6 次重试全部 `503 Service Unavailable`，输出为空。
2. **第二次尝试（用户指定「再试一次自动化通路」，2026-09-12）**
   - `describe_image`（对首行放大图 `f008-sample-p1-line1.png`，整页 36,309 B / 裁剪 7,975 B，PNG 签名已校验 `PNG sig ok: true`）：再次返回 **「未收到图片」**，与首次同因，非路径问题。
   - 视觉队友（`cpa-responses/glm-5.3-flash`，[vision]，`taskType=review`）：`read` 两次调用 + 一次重试全部 `Permission relay send-failed: write EPIPE`；结构化返回 `received_image=false`、`verdict="未收到图片。...错误原文：\"Permission relay send-failed: write EPIPE.\""`，并**明确拒绝描述未看到的画面**。发布物 `agent://edbd5c0e-a2d0-47df-ae80-2ce3baaa499c`。
   - 结论：**自动化通路在本环境内不可用**（同一 EPIPE 故障连续阻断三批子进程工具调用）。最终由用户在 §4.1 完成目视判定。
