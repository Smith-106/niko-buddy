...
**v2.6.6**（2026-08-27 发布，维护发版 notes-only：性能基线刷新 + 仓库清理）

前往 [GitHub Releases](https://github.com/Smith-106/niko-buddy/releases) 下载最新 Windows 安装包（macOS/Linux planned）。

### v2.6.5 更新亮点（2026-08-26，评分缺口补足）

- ✅ **评分核心（D1）** — severity 硬否决/建议两档 + 降级封顶（降级永不触发硬否决）+ 四维清单状态机
- ✅ **旧分迁移（D2）** — 旧分保留 + legacy 显式标记（不重算不追溯）+ 幂等纯函数迁移
- ✅ **文档三件套（D3）** — 19 项术语→白话映射 + [STUB] 图例 + 重标定对照表
- ✅ **运行保障（D4）** — 申诉回执三块（因子链/基线版本/对照锚点）+ 稳定性门 N≥3 差≤0.5
