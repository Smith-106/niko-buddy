...
## 检索强化锚点（共识计划 r2：R-1→R3 已落地；r3：B1–B5 已落地，2026-09-17 同步）
...
| R3-c/d 工程预案占位 | `src/lib/novel/retrieval-scale-placeholders.ts`（ANN 预案占位 implemented=false；矿脉管道契约 ⓪ 计价待定 enabled=false + isOrePipelineUsable 纯函数；**B3-a 后分片预案 implemented=true**）+ `retrieval-scale-placeholders.spec.ts` |
| B1 内部非同源评估集 | `src/lib/novel/internal-eval-set.spec.ts`（判定核：义务召回率 + Wilson CI 下界裁决 + 毒块提取期双证 + 面内 veto 负向控制；门控 `INTERNAL_EVAL_SET=1` → `docs/p0/non-same-source-<date>.md`）+ 生成器 `scripts/build-internal-eval-set.mjs`（`--expand` / 默认 derive / `--check`；断言 N≥200/流派≥4/gold 不落 corpus/与 golden-34 零重叠）+ 产物 `src/lib/novel/kb/internal-eval-queries.json`（fixture，含两份丢弃审计清单）与 `kb/internal-eval-set.generated.json`（qrels，`source=reference-library-self-built`；**非第三方裁决、非 real 门锚点**） |
| B2 λ 标定证据（不翻位） | `src/lib/novel/lambda-calibration.ts`（fitLambda 最小二乘 slope=λ；`LAMBDA_MIN_POINTS=30` 硬抛错 / `LAMBDA_R2_MIN=0.5` / `LAMBDA_MIN_SIGNAL_RATIO=0.1` 近零信号标「不可采信」）+ `lambda-calibration-harness.spec.ts`（双臂实测；门控 `LAMBDA_CALIBRATION=1` → `docs/p0/lambda-calibration-<date>.md`）；**λ=25 ms/pp 与 `calibrated=false` 保持不变** |
| B3-a 检索分片 | `src/lib/novel/shard-routing.ts`（resolveShards / selectShardItems（仅给 perShardTopK 时排序截断，否则保序）/ mergeShards（shard-major 首现序 + `type\0path` 去重）/ applyShardRecall）+ 接线 `search-adapter.ts` novelMixedSearch 可选 `shards`（不传=字节等价；RRF→filterAuthoritative→截断次序与六 stage span 契约序不变；分片观测为 sources span 标量字段） |
| B3-b ANN 接口 + 精确参考 | `src/lib/novel/ann/ann-index.ts`（`AnnIndex` 契约 build/search/descriptor/size + strict schemas + recallAtK + withInjectedClock（注入时钟，零墙钟））+ `ann/brute-force-index.ts`（createExactAnnIndex，exact=true；score desc→id asc）+ `ann/ann-recall-harness.spec.ts`（门控 `ANN_HARNESS=1` → `docs/p0/ann-recall-latency-<date>.md`）；**近似索引未实现，ANN_PLAN.implemented=false 与谓词 `ann_no_implementation` 均不变** |
| B4 矿脉计价骨架 | `src/lib/novel/ore-pricing.ts`（三候选 per-token/per-entry/subscription + estimateOreCost / checkOreQuota（跨币种 fail-loud）+ formatOrePricingSkeleton；费率必传、**不内置任何单价**）+ `ore-pricing.spec.ts`；`pricing.status="pending"` / `enabled=false` / `isOrePipelineUsable()=false` **全部保持** |
| hub 侧内容包与生成器（不入 QMAI 锚点表主链） | `../scripts/gen-xianxia-content-pack.mjs`（CC0 内容包生成器，`--genre xianxia\|fantasy\|wuxia\|scifi\|all`）+ `../scripts/apply-xianxia-overrides.py` / `../scripts/apply-genre-overrides.py`（KB_OVERRIDES 写入；现役 102 项）+ `../scripts/build-reference-index.js`（CLASSIFY/META 表）+ `../scripts/build-reference-kb-view.js`（ROUTING_AGENT/ROUTING_ENG 矩阵真源）→ 产物 `REFERENCE-INDEX.json`（211 项）/ `REFERENCE-KB-VIEW.json` → QMAI 消费面 `src/lib/novel/kb/kb-routing-view.generated.json`（builtFrom `sha256:df39ecbc9a177c41`；禁止手改，由 `scripts/sync-kb-view-to-niko-buddy.mjs` 重生成） |
