...
/**
 * All changelog entries, newest first — single source of truth for the
 * in-app changelog. Consolidated from the pre-refactor per-version consts
 * (60) plus the legacy CHANGELOG array (4); declaration order is
...
export const ENTRIES: ChangelogEntry[] = [
  {
    ...
    highlights: {
      en: [
        ...
        "Engineering convergence batch backfill (15 post-tag commits, exec-plan-20260908): P1-1 analytics-worker CI gate wired (0c8f7a25); P1-2 patterns.cjs eval sandbox replaced by a direct ESM import, new Function count 0 (02036e2e); P1-6 T18 barrel import surface (55aab6b7, 458 app imports moved) with the lint ratchet at 162-to-100 (a39be4f3); T18-G boundaries gate migrated to v7 file categories with positive/negative fixtures and promoted to a hard gate, closing the ratchet at 100-to-0 so --max-warnings is gone and lint reports 0 problems (82085c72); P2 batch: app-side Python zeroed (fecc618f), inline PS1 blocks sunk to Node with a single-sourced signing key (5a852d3e), .test.ts whitelist converged to zero (f3c921fc), bench baseline write decoupled from tracked source (60e3ca81), retry sunset pinned to v2.9.0 (ce942222); B4 Node-20 EOL clearance with 0 node20 action runtimes left (5d46d2aa); CI defect fix for an uncleared timer in llm-provider-section (9bf01382); M1 gate wording fix removing a literal new Function from a comment (be2b9415).",
        "R4 e2e cold-start fix (e690bbf3): the e2e webServer now serves the built app (vite build and vite preview) instead of vite dev, whose first request transformed the whole module graph on demand; both 120s per-test timeout overrides are gone and the suite timeout is a regular 45s. CI e2e step: ubuntu 4.25min to 0.25min, windows failure (6.21min) to success (1.91min). R2 release-chain self-check (f0a4a5fa): build.yml asserts protoc on all three platforms so a broken Windows toolchain fails one explicit step.",
      ],
      zh: [
        ...
        "工程收敛批次补录（15 个 post-tag commits，exec-plan-20260908）：P1-1 analytics-worker CI 门接线（0c8f7a25）；P1-2 patterns.cjs eval 沙箱改为直接 ESM 导入，new Function 归零（02036e2e）；P1-6 T18 barrel 导入面（55aab6b7，458 处 app 导入迁移）+ 棘轮 162→100（a39be4f3）；T18-G boundaries 门禁迁移 v7 文件类别 + 正负 fixture 并升为硬门，棘轮终局 100→0（`--max-warnings` 删除、lint 0 problems）（82085c72）；P2 批次：app 侧 Python 归零（fecc618f）、PS1 内联块下沉 Node + 签名键单源（5a852d3e）、`.test.ts` 白名单收敛至 0（f3c921fc）、bench 基线写盘与受控源解耦（60e3ca81）、retry sunset 钉 v2.9.0（ce942222）；B4 Node-20 EOL 清偿后 0 个 node20 action 运行时（5d46d2aa）；llm-provider-section 未清理定时器的 CI 缺陷修复（9bf01382）；M1 门禁注释件去字面 new Function（be2b9415）。",
        "R4 e2e 冷启动根治（e690bbf3）：e2e 服务改「构建产物」（vite build + vite preview）替代 vite dev（首个请求按需 transform 整图）；两处 120s 单测超时特例删除，套件超时收回常规 45s。CI e2e 步骤：ubuntu 4.25min→0.25min、windows 失败（6.21min）→成功（1.91min）。R2 发版链自检（f0a4a5fa）：build.yml 三平台统一断言 protoc，Windows 工具链缺失改为显式一步失败。",
      ],
    },
  },
...
/**
 * Versions that shipped without visible changelog entries:
 * - 2.2.1-2.2.6, 2.1.1-2.1.10, 2.0.1-2.0.12: patch ranges whose notes were
 *   folded into the next visible release (2.2.0/2.1.0/2.0.0 entries);
 * - 1.0.8-1.0.32: merged into the single 1.0.7 release.
 * Semantics identical to the pre-refactor regex guards.
 */
const HIDDEN_VERSIONS: ReadonlySet<string> = new Set([
  "2.2.1",
  "2.2.2",
  "2.2.3",
  "2.2.4",
...
function isHiddenVersion(version: string): boolean {
  if (HIDDEN_VERSIONS.has(version)) return true
  if (isMergedOnePointRelease(version)) return true
  return false
}

/**
 * Exact-version changelog lookup. Returns [] for hidden versions and for
 * versions without an entry. Behavioral equivalent of the pre-refactor
 * 60-branch if-chain plus the trailing hidden-version guards and the
 * CHANGELOG fallback filter.
 */
export function currentVersionChangelog(version: string): ChangelogEntry[] {
  if (isHiddenVersion(version)) return []
  return ENTRIES.filter((entry) => entry.version === version)
}

/**
 * Full visible version history, newest first. ENTRIES is already
 * descending, so the visibility window is the only filter.
 *
 * Parity note: the pre-refactor list was hand-maintained and frozen at
 * 2.6.3 — 2.7.x and 2.6.4-2.6.13 entries exist (served by
 * currentVersionChangelog) but were never appended to the history, and
 * the intermediate 2.4.7/2.4.8 entries were skipped. This window is
 * preserved verbatim for behavioral parity.
 */
const HISTORY_CEILING = "2.6.3"
const HISTORY_SKIPPED_VERSIONS: ReadonlySet<string> = new Set(["2.4.8", "2.4.7"])
...
export function allChangelog(): ChangelogEntry[] {
  return ENTRIES.filter((entry) => {
    if (isHiddenVersion(entry.version)) return false
    if (HISTORY_SKIPPED_VERSIONS.has(entry.version)) return false
    return compareVersions(entry.version, HISTORY_CEILING) <= 0
  })
}
