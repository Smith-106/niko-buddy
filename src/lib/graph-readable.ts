...
const TYPE_LABELS: Record<string, string> = {
  ...
  entity: "实体", concept: "概念", source: "素材",
  query: "问答记录", synthesis: "综合整理", overview: "总览",
  comparison: "对比", other: "其他",
}

const RELATION_LABELS: Record<string, string> = {
  APPEARS_IN: "出场于", HAPPENS_IN: "发生于", KNOWS: "知晓",
  OWNS: "持有", BELONGS_TO: "隶属于", OPPOSES: "对立",
  SUPPORTS: "支持", AFFECTS: "影响", CAUSED_BY: "源于",
  FORESHADOWS: "伏笔指向", REVEALS: "揭示",
}

const RISK_LABELS: Record<string, string> = {
  foreshadowing: "需追踪", secret: "需核对",
  "canon-rule": "不可违背", "timeline-point": "需校验", conflict: "需推进",
}

const RISK_STATE_LABELS: Record<string, string> = {
  foreshadowing: "未回收", secret: "未揭露",
  "canon-rule": "稳定", "timeline-point": "需校验", conflict: "待推进",
}
...
export function getGraphNodeTypeLabel(type: string): string { return TYPE_LABELS[type] ?? type }
export function getGraphRelationLabel(relation: string | undefined): string { return relation ? RELATION_LABELS[relation] ?? relation : "关联" }
export function getGraphNodeRiskLabel(type: string): string | null { return RISK_LABELS[type] ?? null }
export function getGraphNodeRiskStateLabel(type: string): string | null { return RISK_STATE_LABELS[type] ?? null }
export function getGraphNodeRiskStateOptions(type: string): string[] { return RISK_STATE_OPTIONS[type] ?? [] }
...
export function setGraphNodeRiskStateInContent(content: string, state: string): string {
  if (/^状态：.*$/m.test(content)) return content.replace(/^状态：.*$/m, `状态：${state}`)
  const titleMatch = content.match(/^# .*(?:\r?\n)+/)
  if (titleMatch) return content.replace(titleMatch[0], `${titleMatch[0]}状态：${state}\n\n`)
  return `状态：${state}\n\n${content}`
}

// ── Internal helpers ───────────────────────────────────────────────

function typeLabel(t: string): string { return getGraphNodeTypeLabel(t) }
function relationLabel(r: string | undefined): string { return getGraphRelationLabel(r) }
function wikiLink(label: string): string { return `[[${label}]]` }
function nodeLabel(nodes: GraphNode[], id: string): string { return nodes.find((n) => n.id === id)?.label ?? id }
...
export function groupGraphDocumentNodes(nodes: GraphNode[]): Array<{ title: string; nodes: GraphNode[] }> {
  ...
  for (const group of DOCUMENT_GROUPS) {
    const matched = nodes.filter((n) => group.types.includes(n.type))
    if (matched.length === 0) continue
    groups.push({ title: group.title, nodes: matched })
    for (const n of matched) used.add(n.id)
  }
  const remaining = nodes.filter((n) => !used.has(n.id))
...
export function filterGraphDocumentNodes(nodes: GraphNode[], nodeType: string): GraphNode[] {
  return nodeType === "all" ? nodes : nodes.filter((n) => n.type === nodeType)
}

export function filterGraphDocumentNodesBySearch(nodes: GraphNode[], query: string): GraphNode[] {
  const kw = query.trim().toLowerCase()
  return kw ? nodes.filter((n) => `${n.label} ${n.path ?? ""}`.toLowerCase().includes(kw)) : nodes
}
...
export function filterGraphDocumentNodesByRiskState(nodes: GraphNode[], riskStateOverrides: Record<string, string>, selectedRiskState: string): GraphNode[] {
  ...
  return nodes.filter((n) => (riskStateOverrides[n.id] ?? getGraphNodeRiskStateLabel(n.type)) === selectedRiskState)
}

export function filterGraphDocumentNodesByRelations(nodes: GraphNode[], edges: GraphEdge[], hideUnrelated: boolean): GraphNode[] {
  return hideUnrelated ? nodes.filter((n) => edges.some((e) => e.source === n.id || e.target === n.id)) : nodes
}

export function filterGraphDocumentNodesByIsolation(nodes: GraphNode[], edges: GraphEdge[], showOnlyIsolated: boolean): GraphNode[] {
  return showOnlyIsolated ? nodes.filter((n) => !edges.some((e) => e.source === n.id || e.target === n.id)) : nodes
}

export function getGraphDocumentIsolationStats(nodes: GraphNode[], edges: GraphEdge[]): GraphDocumentIsolationStats {
  return { total: nodes.length, isolated: filterGraphDocumentNodesByIsolation(nodes, edges, true).length }
}
...
export function getGraphDocumentNodeTypeOptions(nodes: GraphNode[]): Array<{ value: string; label: string }> {
  ...
  return [{ value: "all", label: "全部类型" }, ...types.map((t) => ({ value: t, label: getGraphNodeTypeLabel(t) }))]
}
...
export function getGraphDocumentRiskStateOptions(nodes: GraphNode[], riskStateOverrides: Record<string, string>): Array<{ value: string; label: string }> {
  const states = Array.from(new Set(nodes.map((n) => riskStateOverrides[n.id] ?? getGraphNodeRiskStateLabel(n.type)).filter(Boolean) as string[]))
  return [{ value: "all", label: "全部状态" }, ...states.map((s) => ({ value: s, label: s }))]
}
...
export function buildGraphRiskSummaryItems(nodes: GraphNode[], riskStateOverrides: Record<string, string>): GraphRiskSummaryItem[] {
  return getGraphDocumentQuickRiskFilters().map((filter) => ({
    ...filter,
    count: nodes.filter((n) => n.type === filter.nodeType && (riskStateOverrides[n.id] ?? getGraphNodeRiskStateLabel(n.type)) === filter.riskState).length,
  }))
}
...
export function buildGraphRiskReport(nodes: GraphNode[], riskStateOverrides: Record<string, string>): string {
  const riskTypes = new Set(["foreshadowing", "secret", "canon-rule", "timeline-point", "conflict"])
  const riskNodes = nodes.filter((n) => riskTypes.has(n.type))
  const lines: string[] = ["# 风险排查报告", "", `生成时间：${new Date().toISOString().slice(0, 10)}`, "", `共 ${riskNodes.length} 个风险追踪节点。`, ""]
  ...
  for (const node of riskNodes) {
    ...
    if (riskStateOverrides[node.id] !== undefined && defaultState !== state) {
      lines.push(`- 变更：${defaultState} → ${state}`, "")
    }
    if (node.path) lines.push(`- 档案：${node.path}`, "")
    lines.push("")
  }
  return lines.join("\n")
}
...
export function sortGraphDocumentNodes(nodes: GraphNode[], mode: GraphDocumentSortMode): GraphNode[] {
  ...
  const sorted = [...nodes]
  if (mode === "links-desc") { sorted.sort((a, b) => (b.linkCount - a.linkCount) || a.label.localeCompare(b.label, "zh-CN")); return sorted }
  if (mode === "links-asc") { sorted.sort((a, b) => (a.linkCount - b.linkCount) || a.label.localeCompare(b.label, "zh-CN")); return sorted }
...
function pushNodeSection(lines: string[], node: GraphNode, nodes: GraphNode[], edges: GraphEdge[], heading: string): void {
  const nodeEdges = relatedEdges(edges, node.id)
  const eventEdges = nodeEdges.filter((e) => {
    const otherId = e.source === node.id ? e.target : e.source
    const other = nodes.find((n) => n.id === otherId)
    return other?.type === "event" || other?.type === "chapter" || other?.type === "outline"
  })

  lines.push(`### ${heading} ${wikiLink(node.label)}`, "")
  lines.push("#### 基础信息", "")
  ...
  if (nodeEdges.length === 0) {
    ...
    for (const edge of nodeEdges.slice(0, 12)) {
      ...
      lines.push(`| ${wikiLink(nodeLabel(nodes, otherId))} | ${relationLabel(edge.relation)} | ${isSrc ? "指向对方" : "来自对方"} | ${edge.weight} |`)
    }
    lines.push("")
  }
...
/** Generate a full markdown document from the graph data. */
export function buildGraphDocument(nodes: GraphNode[], edges: GraphEdge[]): string {
  const lines = ["# 小说图谱文档", "", "本文档由当前小说档案页自动生成，用于阅读图谱结构；需要长期生效的修改应写入具体节点档案页。", ""]

  const groups = groupNodes(nodes)
  groups.forEach((group, gi) => {
    lines.push(`## ${gi + 1}. ${group.title}`, "")
    group.nodes.forEach((node, ni) => pushNodeSection(lines, node, nodes, edges, `${gi + 1}.${ni + 1}`))
  })

  lines.push(`## ${groups.length + 1}. 全部关系`, "")
  if (edges.length === 0) {
    lines.push("暂无关系。")
  } else {
...
/** Build a mind-map tree structure from the graph. */
    ...
    root.children.push({
      ...
      children: groupNodes.map((node) => ({
        ...
        children: relatedEdges(edges, node.id).slice(0, 8).map((edge) => {
          const otherId = edge.source === node.id ? edge.target : edge.source
          return { id: `${node.id}:${otherId}`, label: nodeLabel(nodes, otherId), children: [] }
        }),
      })),
    })
  }
