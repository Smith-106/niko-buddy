...
describe("changelog", () => {
  it("shows the latest visible 2.3 and 2.2 releases before earlier releases", () => {
    ...
    // 2.5.0 头部 + 2.4.x 链（2.4.11/2.4.10 发布后整体后移）
    expect(versions.slice(0, 9)).toEqual(["2.5.0", "2.4.11", "2.4.10", "2.4.6", "2.4.5", "2.4.4", "2.4.3", "2.4.2", "2.4.1"])
    expect(versions[9]).toBe("2.4.0")
    expect(versions[10]).toBe("2.3.2")
    expect(versions[11]).toBe("2.3.1")
    expect(versions[12]).toBe("2.3.0")
    // 2.2.x patch chain starts after 2.5.0 + 2.4.x + 2.3.x heads
    expect(versions.slice(13, 33)).toEqual([
