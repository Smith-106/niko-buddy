...
const INTENT_PATTERNS: IntentPattern[] = [
  {
    ...
    patterns: [
      ...
      /写\s*第\s*\d+\s*章/,
      /^(write|generate|create|draft)\s*(chapter|ch\.?)\s*\d+/i,
      /^(write|generate|create)\s*(a\s*)?(new|next)\s*chapter\b/i,
  ...
  },
  {
    ...
    patterns: [
      ...
      /接着(写|往下)/,
      /^(continue|resume)\s*(writing\s*)?(chapter|ch\.?)\s*\d+/i,
      /^(continue|resume)\s*(the\s*)?(current|this|next)\s*chapter\b/i,
    ],
