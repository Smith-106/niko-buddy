...
function getFileTypeInfo(path: string): { icon: typeof FileText; type: string } {
  ...
  if (path.includes("index.md")) return { icon: Layout, type: "索引" }
  if (path.includes("log.md")) return { icon: FileText, type: "日志" }
  return { icon: FileText, type: "文件" }
}
...
export function ActivityPanel() {
  ...
  if (queueSummary.processing > 0 || queueSummary.pending > 0) {
    ...
    statusText = t("activity.queueStatus", { done, total: queueSummary.total })
    if (queueSummary.failed > 0) statusText += t("activity.failedSuffix", { count: queueSummary.failed })
  } else if (runningCount > 0) {
    statusText = t("activity.processing", { title: latestItem?.title ?? "..." })
  } else if (queueSummary.failed > 0) {
    statusText = t("activity.taskFailed", { count: queueSummary.failed })
  } else if (fileSyncProcessing > 0 || fileSyncPending > 0) {
    statusText = t("activity.fileSyncPendingStatus", { count: fileSyncProcessing + fileSyncPending })
  } else if (fileSyncFailed > 0) {
    statusText = t("activity.fileSyncFailedStatus", { count: fileSyncFailed })
  } else if (fileSyncError) {
    statusText = t("activity.fileSyncFailed")
  } else {
    statusText = t("activity.done", { title: latestItem?.title ?? t("activity.allTasksDone") })
  }
