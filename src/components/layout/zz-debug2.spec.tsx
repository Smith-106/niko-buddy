...
vi.mock("@/stores/wiki-store", async (importOriginal) => {
  ...
  return {
    ...actual, useWikiStore: Object.assign((s: any) => s(mocks.wikiState), { getState: () => mocks.wikiState }) 
  }
})
vi.mock("@/stores/import-progress-store", async (importOriginal) => {
  ...
  return {
    ...actual, useImportProgressStore: Object.assign((s: any) => s({ tasks: [], cancelTask: vi.fn() }), { getState: () => ({ tasks: [], cancelTask: vi.fn() }) }) 
  }
})
vi.mock("@/commands/fs", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/commands/fs")>()
  return {
    ...actual, readFile: mocks.readFile, writeFile: mocks.writeFile, listDirectory: mocks.listDirectory, deleteFile: mocks.deleteFile, fileExists: mocks.fileExists, copyFile: mocks.copyFile, openFileLocation: mocks.openFileLocation 
  }
})
vi.mock("@/lib/trash", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/trash")>()
  return {
    ...actual, moveFileToTrash: mocks.moveFileToTrash 
  }
})
vi.mock("@/lib/novel/delete-source-memory", async (importOriginal) => {
  ...
  return {
    ...actual, deleteNovelSourceMemory: mocks.deleteNovelSourceMemory 
  }
})
