...
const ENTRIES = [
  {
    version: "2.4.11", // == __APP_VERSION__ (package.json)
    date: "2026-08-18",
...
describe("ChangelogSection", () => {
  it("zh 语言：当前版本徽标 + 收起/展开长条目", () => {
    ...
    expect(screen.getByText((_, el) => el?.tagName === "SPAN" && el.textContent === "v2.4.11")).toBeInTheDocument()
  ...
  it("en 语言渲染英文条目", () => {
    ...
    expect(screen.getByText("当前版本：v2.4.11")).toBeInTheDocument()
  })
  ...
  it("短条目与空 highlights 无展开按钮", () => {
    ...
    expect(buttons).toHaveLength(1) // 只有 2.4.11 的长条目有
