...
| 波次 | 版本 | 能力 | 核心交付 |
|---|---|---|---|
| — | v2.12.0 | 写作主链能力批次 + 导出闭环 | 章节契约/评审举证硬门/verdict解耦 + 指南针/完结清单/上下文压缩 + style_stats rollup/最小返工集/四维度重评 + 三权分立单章编排 repair-loop + PDF导出确认门/导出历史闭环 + confirm批准执行通道/status interrupted态/本地写作路径/凭据恢复/onboarding纵切面 |
...
| 14 | v2.9.3 | 实体物理分层 + 治理审计沉淀 | wiki/entities 走 entity-subdir-resolver 按 entity_subtype 落 5 子目录（335 实体迁入，扁平字面量路径废弃→spec wiki-entities-resolver-contract）+ graph-relevance frontmatter 收敛 canonical（REV-005）+ detective-board `hsl(var(--primary))` 非法 CSS 修复（UI-001）+ 7 个 odyssey session 全库审计 50 findings 沉淀 spec/knowhow/issue 池（prerelease） |
...
在「设置 → 小说 → 任务提示词与技能」中可按任务自定义 AI 行为，覆盖五类任务：**正文写作 / 大纲生成 / 六维审查 / 章节摘要 / 设定提取**。

- **追加提示词**：拼接到对应任务提示词尾部，注入个人风格约束、题材偏好、世界观硬规则等
- **技能名单覆盖**：填写技能库中的技能名称（逗号分隔），非空时覆盖内置技能选择（正文/大纲/审查三组意图生效，经技能库存在性校验，缺失名单进入诊断面）
- **零风险等价保证**：空配置与旧行为逐字节一致（三层等价设计：resolver 空安全、builder 空直通、plugin 缺省路径不变），并有专项回归 spec 守护；审查门控（Consistency P0）不受追加提示词影响
...
| 不知道如何写好某种文风 | 拆书库提取成熟作品的 9 维文风，直接复用 |
...
> 以下为**引擎层**交付（lib + 测试可达；65 号共识 G2 收口：UI 消费面已全部挂载——影游 Play 面板、翻译工作台、同人合并、封面生成、Webhook 配置、项目医生）：
...
前往 [GitHub Releases](https://github.com/Smith-106/niko-buddy/releases) 下载最新 Windows 安装包（macOS/Linux 规划中，部分 Rust 命令已做平台 no-op 处理，见 v2.4.11 wake-lock）。
...
2. **导入或生成大纲**：导入已有大纲，或通过 AI 生成；也可从拆书库提取文风作为风格约束
3. **配置模型**：在设置中配置 LLM API 地址和密钥；可设置「默认模型」统筹后台任务
4. **开始写作**：在 AI 对话区输入写作指令（如"继续写第 30 章"）
5. **审阅草稿**：查看生成结果，可编辑、重写或审稿
6. **确认保存**：满意后保存为正式章节，系统自动摄取记忆
7. **查看图谱与审查**：在图谱视图查看人物关系；审查中心查看六维评分与角色命中报告
...
│   │   ├── graph/                # 图谱视图
│   │   ├── layout/               # 布局组件（含使用说明入口）
│   │   ├── novel/                # 小说功能组件（灵魂、认知、伏笔、记忆、拆书库等）
│   │   ├── review/               # 审查中心
│   │   ├── search/               # 搜索视图
│   │   ├── settings/             # 设置面板（模型、数据管理、界面等）
│   │   ├── sources/              # 原始资料与大纲管理
│   │   └── ui/                   # 通用 UI 组件
│   ├── lib/                      # 业务逻辑库
│   │   ├── novel/                # 小说核心模块
