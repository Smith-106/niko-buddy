...
- **四轮共识审计**：deepseek-v4-flash + GLM-5.3-flash + qwen3.8-flash 对 7 个此前未复核的高流量面（chat-panel / deep-chapter-generation / chapter-ingest / context-engine / graph-view / preview-panel / review-view）三模型全覆盖；行号门对冻结的派发修订版核验：193 条发现 / 185 条行号验证通过、62 个跨模型共识点；四轮累计修复 **69 项确认缺陷**（24d3db1d、490746de、0b0eb503）。
- **两处数据丢失路径封堵**：preview-panel 读档失败不再把错误文本写进编辑器（防 1s 防抖自动保存用错误串覆盖原文件）；graph-view 档案页读档失败不再回退模板内容进入编辑（保存目标仍指真实档案页，保存即覆盖）。

### odyssey-ui 外壳与宿主接线

- 面板滚动不可达、页面泄露/重叠、MCP 连接状态缺失、英文诊断串直出修复（4c79ee58）。
- 7 个移植面板接入真实宿主（3b919b00）；门与锁挂载进应用壳层（4862dee2）。

### 错误面与 i18n 口径统一

- 用户可感知失败统一走 `formatOperationError`（本地化引导 + 原始诊断；R3 重写 27 处并在 R4 各面推广）。
- 可见坏键 160→0、en 缺译 133→0；zh/en 键集双侧一致各 **2659**。
- 批量 a11y：筛选器/对话框 `aria-label`、`role=alert` 错误横幅、`role=tablist` 容器、`useId`+`htmlFor` 关联。

### 验证纪律

- 3 处反证（重注入缺陷后新断言原样失败）；9 处「把缺陷写成预期」的评审债测试反转；新增回归用例 28+。
- 全量套件绿：831 文件 / **12793** 用例（12 skipped）+ 实机 e2e **84** 通过。
...
- **维度收敛**：核心维 ≤3 仅聚合层裁剪（非删维）；Track B 六维全量保留（明细可展开）；门控中位方差降 ≥15%（同协议同窗 N≥5，跨版本对比锁定基线，维度数归一化对照）；负向集召回双门 ≥基线−2%（收敛不得掩盖检测退化）。
...
> 注（2026-08-31）：stretch gate 四项指标（维度收敛方差/跨模型 Δ/跨语言 F1）为自述目标，v2.7.4 无独立基准报告佐证——deferred（owner=质量门控模块，跟进见 docs/qmai-codex-delivery/16-final-acceptance-framework-20260829.md（stretch gate 证据 deferred 状态）），不阻塞纯应用发版。补证（维度收敛方差/跨模型 Δ/跨语言 F1 独立基准报告）随 L9 脉冲 κ Round-2 一并执行（65 号共识 G10，2026-09-06 挂账）。
>
...
### 对抗纵深波（roadmap v2.7 第二波）

- **D3 探针**：分层三票加权（规则 0.3+嵌入 0.4+一致性 0.3）；检出率 ≥90% 且误报率 ≤5% 双门同报；灰区 [0.4,0.7] 全量转人工复审（边界稳定 ≤1.5×）。
...
### 门控地基波（roadmap v2.7 第一波）

- **门控解耦证明**：同稿 3 模型决策级三元一致率 ≥95%（95%CI 下限 ≥90%，N≥30）；结论翻转（pass↔fail）=0 红线；逐维 5% 容差仅辅助诊断。
- **换模型硬门**：模型指纹/版本/权重变更触发 100%（CI 监听模型清单）；漏报=0（注入阴性样本全捕获）；审计留痕。
- **版本锁**：build 产物哈希（前端 bundle + Rust 二进制）+ 门控配置全谱哈希（prompt/温度/权重）写 manifest；同 commit 两次构建一致；失配 fail-fast（不做静默降级）。
- **冷评全自动结案**：P0/P1 全自动 + P2 异常回退（单章分<8 或波动>1.5σ 回退人工）；零人工结案率 ≥90%，人工兜底 ≤10% 全留痕；审计报告 100% 可追溯；Draft-first 守卫只落 pending/ready。

## [2.6.13] - 2026-08-28

### 检测 W4 对抗压测 + 验收双门认证 + W3 显影补闭环（roadmap 9.5 Phase 3 收口）

- **对抗压测（W4）**：三路攻击（改写/风格迁移/水印剥离）各 N≥30 确定性生成器（seed 可复现）；通过率较 v2.6.12 冻结基线显著下降（配对 p<0.05 AND 降幅≥30%，绝对≥5pp 兜底）；对抗集与验收集零交集（防泄漏过拟合）。
- **双门认证**：四项 AND（中位≥9.5 ∧ 分章 σ<0.3 ∧ 分章 Δ<0.15）——任一章不过整书不过（防均分掩盖单章塌方）；独立复现 ≥9.0（跨种子硬门 ≥3 seed；换模型≥9.0 作泛化补证）；压测前后各跑一次产出衰减对比。
- **隐性价值显影闭环（W3）**：曝光（高亮）→感知（悬浮说明）→采纳（一键回传）三态埋点（覆盖 100%，回写成功率≥99%）+ 熔断降级（双门返工自动降级不阻塞发版）。
- **共识复核**：7 方向共识分复核 ≥9.5（独立评分取中位，ICC≥0.8）。
- **DoD 收口**：清单 100% 勾选 + DEFER 项登记 owner+日期 + 集成验收无 P0/P1 阻断。

## [2.6.12] - 2026-08-28

### 用户 W1-W4 + 测试 W2-W3（roadmap 9.5 Phase 3）

- **知情接受（W1）**：理由卡一键生成（可回溯锚点：来源/证据片段——防虚假归因）+ 知情接受率 ≥90% 硬门（N≥50；分母剔除未展示理由的灰盒事件）。
- **记忆固化主动召回（W2）**：accept 后主动召回相关固化记忆（非打断式侧边提醒）；召回→被 accept 硬口径（防刷量：展示不计数，reject 计负分）+ 成功率 ≥70%（N≥30）。
- **风格脚手架（W4）**：静态签名（句长/对话密度/叙事节奏指纹）作生成期软约束；可选+可关闭（防同质化，不自动重写既有章节）。
- **周度冷评（测试 W2）**：盲评流水线（评者非作者——不与改写链路耦合）；六维 overall 中位 ≥9.0 出关（N≥5）；Consistency(P0) 恒为硬门不得覆盖。
- **变异分水岭（测试 W3）**：影子分支内容变异 kill score ≥80%（语义余弦 <0.85 记有效变异；风格变异仅辅助观测不计门）+ 专项/混沌回归（故障注入限影子环境，0 阻断级失败，白名单豁免已知抖动）。

## [2.6.11] - 2026-08-28

### 域漂移基准 + 跨维交叉门 + 全文窗漂移门 + P0 锁死（roadmap 9.5 Phase 3）

- **域漂移基准（D1）**：(章次,模型,提示词) 三元组版本锚定（防漂移门无参照系）+ cosine 距离 + 训练分布 ±σ 硬基线（超阈即漂）。
- **跨维交叉门（D4）**：矛盾矩阵规则表（thril×pacing / consistency×anti_ai / pull×context）；多信号一致（≥2 矛盾）才判 AI；单维命中降级灰区（不杀）。
- **全文窗漂移门（D5）**：滑窗检测分序列（窗大小 3）；窗间漂移超阈（0.15）整章重检；合法手法（闪回/POV 切换/时间跳跃/梦境）零误杀。
- **P0 锁死（D7）**：状态机「任一 P0 开→LOCKED」；锁触发输出集=P0 项∪D8 未清 Q0 项（禁止静默清零）；仅显式逐项 close 才缩减；严格限定 P0（误扩 P1/P2 瘫痪产出）；Quality 高分不得解锁（对抗性负向测试）。
- **观测**：FDR 监控（D2，EWMA 控制图非硬门）/ 灰区复核（D6，进阈值 0.45/出阈值 0.55，双人盲评 Kappa≥0.7，阻断写作至人工复核）。

## [2.6.10] - 2026-08-28

### 写作 W3-W4 + 编辑 W2-W3（roadmap 9.5 Phase 3）

- **盲测复述（D1）**：一句话复述 ≥95% 命中率（口径冻结：要点召回率≥70% 记命中）+ 距离衰减曲线（长程记忆验证）。
- **伏笔追踪（D2）**：登记→回收闭环（key 归一化+别名词典，置信度≥0.7 才入册）+ 回扣质量二级校验（逐字复述偷懒回扣惩罚）。
- **伏笔差分入 P0（D4）**：纯函数集合运算（机械可判零 LLM）+ 跨段差分（人设漂移检测）；Quality 不得覆盖 P0。
- **独立签字表（D6）**：top/bottom 10% 结论摘要≥50 字+异议栏+原文引用≥10 字且与结论绑定（防只签不评/签而不看）。
- **观测**：回灌 dry-run（D3，只带锚点摘要）/ 双盲三编辑视角分裂（D5）/ 归因反哺（D7）。

## [2.6.9] - 2026-08-28

### 检测链强化（roadmap 9.5 Phase 2）
...
### 写作意图 + 编辑校准（roadmap 9.5 Phase 1）

- **校准基线（D1）**：六维分维分布（N≥5）+ rubric 版本冻结 + 确定性（同输入同输出）。
- **地板闸（D2）**：consistency 9.0 单维一票否决 / anti_ai 8.5 / quality 8.5 不得覆盖 P0；只挡正文回填（草稿修正自由）；n<30 回退 P50 临时闸。
...
- **改写判据（D5）**：复合触发（≥2 软维触地板 且 中位<9.0 才改写）；P0 失败短路抑制改写；可回滚/不可回滚二分类 + 快照锚点。
...
- 性能基线 timestamp 刷新（测试自动产物，无功能变更）。
- 仓库清理：冗余远程分支归档删除、构建产物清理、知识库重新同步。

## [2.6.5] - 2026-08-26

### 评分缺口补足（7 团队共识 · 蓝图 blueprint-v265）
